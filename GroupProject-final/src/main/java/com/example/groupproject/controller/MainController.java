...
//    }
    ...
    public String deleteBook(@PathVariable Long bookId, HttpSession session) {
        ...
        if (loggedInUserId != null) {
            ...
            if (book != null && book.getUser().getId().equals(loggedInUserId)) {
                bookService.deleteBook(bookId);  // Ensure you have this method in your bookService
            }
        }
    ...
    public String getExternalData(Model model) {
        // This is the specific URL from the JS code, including its parameters
        String baseUrl = "https://book-finder1.p.rapidapi.com/api/search?series=Wings%20of%20fire&book_type=Fiction&lexile_min=600&lexile_max=800&results_per_page=25&page=1";
        ...
        try {
            ResponseEntity<ApiDataItem[]> response = restTemplate.exchange(baseUrl, HttpMethod.GET, entity, ApiDataItem[].class);
            ...
            model.addAttribute("error", "Failed to fetch data from the external API. Reason: " + e.getResponseBodyAsString());
        } catch (Exception e) {
